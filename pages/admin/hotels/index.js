// pages/admin/hotels/index.js — Hotel management, card layout
import { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/router';
import AdminLayout from '../../../components/layout/AdminLayout';
import {
  Plus, Search, RefreshCw, Eye, Pencil, Trash2, Hotel,
  MapPin, Star, Loader2, CheckCircle, Building2,
...
function ScoreBadge({ score, label }) {
  ...
  const color = n >= 9 ? 'bg-emerald-600' : n >= 8 ? 'bg-emerald-500' : n >= 7 ? 'bg-blue-600' : 'bg-amber-500';
  return (
    <div className="flex items-center gap-1.5">
      <span className={`${color} text-white font-bold text-sm px-2.5 py-1 rounded-xl leading-none`}>
        {n.toFixed(1)}
      </span>
...
function HotelCard({ hotel, index, onEdit, onDelete, onToggleStatus, onClick }) {
  const gradient = CARD_GRADIENTS[index % CARD_GRADIENTS.length];

  return (
    <div className="bg-white rounded-3xl border border-slate-200 hover:border-emerald-300 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col cursor-pointer group"
      onClick={onClick}>
      ...
      <div className="relative h-40 overflow-hidden">
        ...
        {hotel.review_score && (
          <div className="absolute top-3 right-3">
            <div className={`${Number(hotel.review_score) >= 8 ? 'bg-emerald-600' : 'bg-blue-600'} text-white font-bold text-sm px-2.5 py-1 rounded-xl shadow`}>
              {Number(hotel.review_score).toFixed(1)}
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="mb-3">
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i}
                className={`w-3.5 h-3.5 ${i < (hotel.star_rating || 0) ? 'text-amber-400 fill-amber-400' : 'text-slate-200 fill-slate-200'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4 mt-auto">
          ...
          <div className="bg-emerald-50 rounded-2xl p-2.5 text-center">
            <div className="text-base font-bold text-emerald-700 leading-none mb-0.5">{hotel.review_count || 0}</div>
            <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Reviews</div>
          </div>
          <div className="bg-amber-50 rounded-2xl p-2.5 text-center">
            <div className="text-base font-bold text-amber-700 leading-none mb-0.5">
              {hotel.review_label ? hotel.review_label.slice(0, 4) : '—'}
            </div>
        ...
        <div className="flex gap-2" onClick={e => e.stopPropagation()}>
          ...
          <button onClick={() => onToggleStatus(hotel)}
            className={`w-10 flex items-center justify-center rounded-2xl border transition-all text-xs font-bold ${
              hotel.status === 'active'
...
export default function AdminHotelsPage() {
  ...
  const stats = {
    ...
    avgScore: hotels.length
      ? (hotels.reduce((s, h) => s + Number(h.review_score || 0), 0) / hotels.length).toFixed(1)
      : '—',
  };
  ...
  return (
    <AdminLayout title="Hotels">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { icon: Hotel,       label: 'Total Properties', value: stats.total,    shade: 'bg-emerald-50', text: 'text-emerald-700', icon_c: 'text-emerald-600' },
          { icon: CheckCircle, label: 'Active',           value: stats.active,   shade: 'bg-sky-50',     text: 'text-sky-700',     icon_c: 'text-sky-600'     },
          { icon: Star,        label: 'Featured',         value: stats.featured, shade: 'bg-amber-50',   text: 'text-amber-700',   icon_c: 'text-amber-600'   },
          { icon: TrendingUp,  label: 'Avg Score',        value: stats.avgScore, shade: 'bg-violet-50',  text: 'text-violet-700',  icon_c: 'text-violet-600'  },
        ].map(s => (
      ...
      {/* ── Cards grid ── */}
      {loading ? (
        <div className="flex justify-center items-center py-28 text-slate-400">
          <Loader2 className="w-6 h-6 animate-spin" />
        </div>
      ) : filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-28 bg-white rounded-3xl border border-slate-200 border-dashed text-slate-400">
          <div className="w-20 h-20 bg-slate-100 rounded-3xl flex items-center justify-center mb-5">
            <Hotel className="w-9 h-9 opacity-30" />
          </div>
          <h3 className="text-lg font-bold text-slate-600 mb-1">No hotels found</h3>
          <p className="text-sm mb-5">{search ? 'Try a different search term' : 'Add your first property to get started'}</p>
          {!search && (
            <button onClick={() => router.push('/admin/hotels/new')}
