// pages/admin/dorm-applications/index.js
...
import {
  ...
  Inbox, MapPin, FileText, Users, Filter
} from 'lucide-react';
...
export default function AdminDormApplications() {
    ...
    <AdminLayout title="Dorm Applications">
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">{loading ? 'Loading…' : `${total} total applications`}</p>
        </div>
        <button onClick={load}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-4">
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="border-b-2 border-slate-100 bg-slate-50">
                {['App ID', 'Student', 'Dormitory', 'Room', 'Move-in', 'Docs', 'Status', 'Actions'].map(h => (
              ...
              {loading ? (
                ...
                <tr><td colSpan={8} className="py-20 text-center">
                  <div className="flex flex-col items-center gap-3 text-slate-400">
                    ...
                    <p className="font-semibold text-slate-500">No dormitory applications yet</p>
                  </div>
                </td></tr>
              ) : apps.map(a => (
                <tr key={a.id}
                  onClick={() => router.push(`/admin/dorm-applications/${a.id}`)}
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-slate-800">{a.student_name}</div>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <div className="text-sm font-semibold text-slate-800">{a.hotel_name}</div>
                  ...
                  <td className="px-4 py-4">
                    <span className="text-sm text-slate-600">{a.room_name || '—'}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="text-sm text-slate-600">
                      {a.move_in_date ? new Date(a.move_in_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="text-xs font-bold text-emerald-700">{a.doc_approved || 0}</span>
                    <span className="text-xs text-slate-400">/{a.doc_count || 0} approved</span>
                  </td>
                  <td className="px-4 py-4"><StatusBadge status={a.status} /></td>
                  <td className="px-4 py-4" onClick={e => e.stopPropagation()}>
                    <button onClick={() => router.push(`/admin/dorm-applications/${a.id}`)}
        ...
        {!loading && apps.length > 0 && (
          <div className="px-6 py-3 border-t border-slate-100 bg-slate-50 text-xs text-slate-500">
            Showing <span className="font-bold text-slate-700">{apps.length}</span> of <span className="font-bold text-slate-700">{total}</span>
          </div>
        )}
      </div>
      ...
      {!loading && pages > 1 && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1} className="p-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 disabled:opacity-40 bg-white">
              <ChevronLeft className="w-4 h-4 text-slate-600" />
            </button>
            <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page >= pages} className="p-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 disabled:opacity-40 bg-white">
