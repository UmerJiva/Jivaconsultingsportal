...
export default function UniversityDetail() {
  ...
  const role = router.pathname.startsWith('/admin') ? 'admin' : router.pathname.startsWith('/student') ? 'student' : 'agent';
  ...
  // Build photo grid: up to 5 photos shown in the mosaic layout
  const gridPhotos = photos.slice(0, 5);
  // Placeholder gradient colors for missing slots
  ...
  return (
    <AdminLayout title={uni.name}>
      <div className="max-w-full">
        <button onClick={()=>router.push('/admin/universities')} className="flex items-center gap-2 text-sm text-slate-500 hover:text-brand-600 mb-4 transition-colors">
                        ...
                        <div key={p.id} className="bg-white border border-slate-200 rounded-2xl p-5 hover:border-brand-300 hover:shadow-sm transition-all">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <button onClick={() => router.push(`/admin/program/${p.id}`)}
                                ...
                                View Program <ExternalLink className="w-3.5 h-3.5"/>
                              </button>
                              <button onClick={() => router.push(`/admin/program/${p.id}/requirements`)}
                                className="flex items-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-600 font-bold px-3 py-2.5 rounded-xl text-sm transition-colors border border-slate-200">
                                Requirements
                              </button>
                            </div>
                          </div>
