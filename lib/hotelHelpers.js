// lib/hotelHelpers.js — Shared hotel DB helpers (used inside transactions)
// conn here is a mysql2 single Connection (not pool), from mysql.createConnection()
  ...
  if (Array.isArray(images) && images.length > 0) {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      await conn.execute(
        ...
        [hotelId, img.image_url.trim(), img.caption || null, img.is_primary ? 1 : 0, i]
      );
    }
  }
      ...
      if (room.id) {
        ...
        await conn.execute(
          ...
          [
            ...
            room.size_sqm || null, room.max_guests || 2,
            room.bed_type || null, room.view_type || null,
            room.price_per_night || 0, room.original_price || null,
            room.discount_percent || 0,
            room.free_cancellation ? 1 : 0,
            room.cancellation_deadline || null,
            room.no_prepayment ? 1 : 0,
